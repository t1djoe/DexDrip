...
public class WixelReader  extends Thread {
    ...
    public static void sStop() {
        ...
        try {
            ...
            Log.e(TAG, "caught InterruptedException, could not wait for the wixel thread to exit");
        }
        sStarted = false;
    ...
    public static List<TransmitterRawData> Read(String hostName,int port, int numberOfRecords)
    {
        ...
        try
        {
            ...
            CommunicationHeader ch = new CommunicationHeader();
            ch.version = 1;
            ch.numberOfRecords = numberOfRecords;
            String flat = gson.toJson(ch);
            CommunicationHeader ch2 = gson.fromJson(flat, CommunicationHeader.class);
    ...
    public void runFake()
    {
        ...
        while (!mStop) {
            try {
                ...
                setSerialDataToTransmitterRawData(fakedRaw, 100, 100, new Date().getTime());
